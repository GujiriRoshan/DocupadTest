...
export default class CustomLookupSearch extends LightningElement {
  ...
  getLookupResult() {
    ...
    fetchSectionsAndTemplateDetails({ searchKey: this.searchKey, objectName: this.objectApiName })
      ...
      .then((result) => {
        //console.log('searchKey'+this.searchKey.length);///comment by saran
        //console.log('searchKey new one '+this.searchKey.length == );
        //console.log('true or falsse:'+this.searchKey.length  3);
        if (result.length === 0 && (this.searchKey == null) ) {
          ...
          console.log('Record Lists '+this.searchKey.length);
